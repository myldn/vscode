...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	open(openConfig: IOpenConfiguration): ICodeWindow[] {
		...
		// When run with --merge, take the first 3-4 files to open as files to merge
		if (openConfig.mergeMode && filesToOpen && filesToOpen.filesToOpenOrCreate.length >= 3) {
		...
		if (filesToOpen && openConfig.waitMarkerFileURI) {
			filesToOpen.filesToWait = { paths: [...filesToOpen.filesToDiff, ...filesToOpen.filesToMerge, ...filesToOpen.filesToOpenOrCreate], waitMarkerFileUri: openConfig.waitMarkerFileURI };
		}
	...
	private doOpen(
		...
		if (filesToOpen && potentialNewWindowsCount === 0) {
			...
			const fileToCheck = filesToOpen.filesToOpenOrCreate[0] || filesToOpen.filesToDiff[0] || filesToOpen.filesToMerge[0];
